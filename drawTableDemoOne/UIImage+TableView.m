...
- (instancetype)initWithParam:(CGFloat)width
           tableHeaderIdArray:(NSArray *)tableHeaderIdArray
         tableHeaderNameArray:(NSArray *)tableHeaderNameArray
               tableDataArray:(NSArray *)tableDataArray {
  /**
   *  设置文字大小
   */
   ...
   *  表头颜色
   */
  UIColor *titleColor = [UIColor colorWithRed:91 / 255.0
                                        green:155 / 255.0
                                         blue:213 / 255.0
                                        alpha:1];
  /**
   *  奇数行背景色
   */
  UIColor *rowColor_1 = [UIColor colorWithRed:210 / 255.0
                                        green:222 / 255.0
                                         blue:239 / 255.0
                                        alpha:1];
  /**
   *  偶数行背景色
   */
  UIColor *rowColor_2 = [UIColor colorWithRed:234 / 255.0
                                        green:239 / 255.0
                                         blue:247 / 255.0
                                        alpha:1];
  /**
  ...
  //计算表格每一行的y位置并保存
  {
    float biaoTouHeight = 0; //表头最大高度
                             //获取表头最大高度
    float cellWidth = tableWidth / tableHeaderNameArray.count -
                      marginLeftRight; //表格每列的宽度
    for (int i = 0; i < tableHeaderNameArray.count; i++) {
      CGSize stringSize =
          [tableHeaderNameArray[i] sizeWithFont:font
    ...
    [rowPositionYArray
        addObject:[NSString stringWithFormat:@"%f", biaoTouHeight]];
      ...
      if (index < tableDataArray.count) {
        ...
        for (int num = 0; num < collNum; num++) {
          NSString *value = [NSString
              stringWithFormat:@"%@",
                               [dic objectForKey:tableHeaderIdArray[num]]];
          NSAttributedString *attributedText = [[NSAttributedString alloc]
              initWithString:value
                  attributes:@{NSFontAttributeName : font}];
          CGSize stringSize =
              [attributedText
                  boundingRectWithSize:CGSizeMake(collWidth - marginLeftRight,
                                                  999)
        ...
        [rowPositionYArray
            addObject:[NSString stringWithFormat:@"%f", currentRowMaxHeight]];
  ...
  UIGraphicsBeginImageContext(
      CGSizeMake(tableWidth, tableHeight));         // 创建内存中的图片
  ...
  for (int num = 0; num < collNum; num++) {
    ...
    CGSize size = CGSizeMake(
        collWidth - marginLeftRight,
        currentRowY); // collWidth-marginLeftRight是为了计算留出左右边距

    NSAttributedString *attributedText = [[NSAttributedString alloc]
        initWithString:valueName
            attributes:@{NSFontAttributeName : font}];
    CGSize valnueNameSize =
        [attributedText
    ...
    [valueName
            drawInRect:CGRectMake((collWidth - valnueNameSize.width) / 2 +
                                      num * collWidth,
                                  2 + (currentRowY - valnueNameSize.height) / 2,
                                  valnueNameSize.width, valnueNameSize.height)
        withAttributes:dict];
  }
  ...
  for (int index = 0; index < tableDataArray.count; index++) {
    ...
    for (int num = 0; num < collNum; num++) {
      NSString *value = [NSString
          stringWithFormat:@"%@", [dic objectForKey:tableHeaderIdArray[num]]];
      CGSize size =
          CGSizeMake(collWidth - marginLeftRight,
                     currentRowMaxHeight); // collWidth-8是为了计算留出左右边距
      NSAttributedString *attributedText = [[NSAttributedString alloc]
          initWithString:value
              attributes:@{NSFontAttributeName : font}];
      CGSize valnueNameSize =
      ...
      [value drawInRect:CGRectMake(
                            0 + (collWidth - valnueNameSize.width) / 2 +
                                num * collWidth,
                            (currentRowMaxHeight - valnueNameSize.height) / 2 +
                                currentRowY,
                            valnueNameSize.width, valnueNameSize.height)
          withAttributes:dict]; //画cell文字
    }

    //画行分割线
    CGContextSetStrokeColorWithColor(ctx, [UIColor whiteColor].CGColor);
    CGPoint line[] = {CGPointMake(0, currentRowY),
                      CGPointMake(tableWidth, currentRowY)};
    CGContextStrokeLineSegments(ctx, line, 2);
    currentRowY = currentRowY + currentRowMaxHeight;
  }
  //=====画列分割线======
  CGContextSetStrokeColorWithColor(
      ctx, [UIColor whiteColor].CGColor); //设置列分割线颜色
  for (int index = 1; index < collNum; index++) {
    CGPoint line1[] = {CGPointMake(0 + index * collWidth, 2),
                       CGPointMake(index * collWidth, tableHeight)};
